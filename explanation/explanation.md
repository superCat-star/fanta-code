...
In this competition, we design a method to solve the SMARTS autonomous driving tasks as described in the below figure. Given the information from the SMARTS ENVIRONMENT, a Meta Controller decides whether to pass the information to the Scheduler to make further decisions. If the Scheduler is activated, then it will arrange the specific driving decisions and return it to the environment. Otherwise, a direct stop decision is returned to the environment.

<img src="explain of the method/figs/image-20221108115424782.png" alt="image-20221108115424782" style="zoom:50%;" />

In detail, the proposed method is a hierarchical framework.  The overall framework for solving the driving tasks as shown in the figure below consists of two modules, the upper-level module and the lower-level module. The upper-level module (Meta Controller) is in charge of deciding whether the ego should move or keep static. This module consists of an **offline learning agent** trained via RvS (Reinforcement learning Via Supervised learning) [1] and a rule-based collision-detection module.  The lower-level module （Scheduler) maintains a policy repository consisting of three kinds of policies to make decisions about the ego's speed, heading and whether to change to a different lane. 

<img src="explain of the method/figs/image-20221106182845682-1667730530248-5.png" alt="image-20221106182845682" style="zoom:80%;" />

Our source code is available here (https://github.com/superCat-star/fanta-code). **You can obtain our docker image through the following command:**

```
docker pull teafrappuccino/docker_project:1.0.0
```
...
However, as the actual driving scenarios are so complicated, this simple collision-checking module is not able to detect all the potential risks. Therefore,  we train an offline agent via RvS using the officially provided Waymo motion datasets due to its powerful ability over pure offline reinforcement learning in situations that rewards are hard to define [1]. The RvS model and collision-detection rule vote on whether the ego should be static or moving in the next step, and both of them have a one-vote veto to keep the ego static for security consideration.  During the experiments, we observed that the RvS contributes a lot in avoiding the accident in many key states such as the intersections in turning and merging scenarios. Without the RvS model, the collision-checking module can always malfunction.  Meanwhile, as the training of RvS may be insufficient or underfitting due to the data imbalance, this simple collision-checking module can also prevent the RvS agent from making stupid mistakes in some simple scenarios.
...
$$
\max \_\theta \sum\_{\tau \in \mathcal{D}} \sum\_{1 \leq t \leq|\tau|} \mathbb{E}\_{\omega \sim f\left(\omega \mid \tau\_{t: H}\right)}\left[\log \pi\_\theta\left(a\_t \mid s\_t, \omega\right)\right]
$$


The overall algorithm is described in the following figure:

<img src="explain of the method/figs/image-20221106170153165-1667725315467-1-1667725318663-3.png" alt="image-20221106170153165" style="zoom:67%;" />

## 2. 2 MDP

State: The states include the bounding box of ego, the speed limit of the current lane, the relative heading, position and speed of the five closest vehicles to ego, and the bounding box of these closest vehicles.
...
Goal: We set the output of $f(\omega \mid \tau)$ to be the waypoints and headings in the subsequent next 5 steps, which is the short-term goal position the ego is about to visit. 
